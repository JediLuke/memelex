defmodule Memelex.Keymaps.UserInputHandler do
   ...
   def process(%{root: %{active_app: :memex}} = radix_state, @tab_key) do
      ...
      case find_open_tidbit(radix_state.memex) do
         ...
         [%{gui: %{mode: :edit, focus: :body}} = t] ->
            ...
            :ok
         _otherwise ->
            Logger.warn "No TidBits open in `:edit` mode."
            :ignore
      end
   end
...
# #   end
   ...
   def find_open_tidbit(%{story_river: %{open_tidbits: tidbit_list}}) do
      #TODO change this so that instead we have a field saying which is the `active` TidBit, even if multiple are in edit mode
      tidbit_list |> Enum.filter(& &1.gui.mode == :edit) 
   end
